...
import com.atenea.sdr.contest.algorithm.IncrementalSDR;
import com.atenea.sdr.contest.algorithm.SDRAlgorithm;

public class IncrementalSDRThread implements Runnable {
	private final static Logger LOGGER = Logger.getLogger(IncrementalSDRThread.class.getName());
	...
	public IncrementalSDRThread(MainThread main) {
		this.main = main;
	}
			...
			while (true) {
				if (incremental) {
					...
					if (!vertices.isEmpty()) {
						graph = main.getGraph();
						subgraph = (Graph) graph.traversal().E().subgraph("g").cap("g").next();
						ids.addAll(IncrementalSDR.updateWeight(vertices, subgraph, queryTraversal)
