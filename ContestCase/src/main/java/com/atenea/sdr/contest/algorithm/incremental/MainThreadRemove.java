...
public class MainThreadRemove implements Runnable {
	...
	public void startComputing() throws IOException, InterruptedException {
		...
		try (InputStream input = new FileInputStream("src/main/resources/config.properties")) {
			...
			List<Vertex> answerList = graph.traversal().V().hasLabel("Answer").order().by("date").toList();
			...
			for (int x = 0; x < records; x++) {
				...
				verticesToUpdate.addAll(graph.traversal().V(answerList.get(x).id()).both().toList());
				

				getGraph().traversal().V(answerList.get(x).id()).property("delete","true").iterate();
