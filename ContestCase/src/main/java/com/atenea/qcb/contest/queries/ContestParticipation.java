...
public class ContestParticipation {
	private final static Logger LOGGER = Logger.getLogger(ContestParticipation.class.getName());
	...
	public void run(Graph graph, FileHandler fh, Boolean subgraph) throws IOException {
		...
		for (int i = 0; i < 6; i++) {
			...
			List<Vertex> result =
					graph.traversal().V().where(__.in("askedTo").in("formulates").has("idContest", 508)).dedup()
							.toList();
