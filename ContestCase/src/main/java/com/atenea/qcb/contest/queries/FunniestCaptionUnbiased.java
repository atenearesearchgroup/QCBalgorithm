...
public class FunniestCaptionUnbiased {

	private final static Logger LOGGER = Logger.getLogger(FunniestCaptionUnbiased.class.getName());
	...
	public void run(Graph graph, FileHandler fh, Boolean subgraph) throws IOException {
		...
		for (int i = 0; i < 6; i++) {
			...
			List<Collection<Object>> result = graph.traversal().V().as("caption").in("contains")//.coin(0.95)
