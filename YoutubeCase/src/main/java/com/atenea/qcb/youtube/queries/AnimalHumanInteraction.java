...
public class AnimalHumanInteraction {
	private final static Logger LOGGER = Logger.getLogger(AnimalHumanInteraction.class.getName());
	...
	public void run(Graph graph, FileHandler fh, Boolean subgraph) throws IOException {
		...
		for (int i = 0; i < 6; i++) {
			...
			List<Vertex> result = graph.traversal().V().and(__.out("contains").has("className","person"), __.out("contains").has("className",P.within("cat","dog","bird","zebra","cow","bear","horse","giraffe","elephant"))).toList();
